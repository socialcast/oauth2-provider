  ...
  describe "#initialize" do
    ...
    describe "with a valid client_id and redirect_uri" do
      it "doesn't raise any exception" do
        lambda {
          OAuth2::Provider::Rack::AuthorizationCodeRequest.new(@valid_params, rack_env)
    ...
    describe "without a client_id" do
      it "raises OAuth2::Provider::Rack::InvalidRequest" do
        lambda {
          OAuth2::Provider::Rack::AuthorizationCodeRequest.new(@valid_params.except('client_id'), rack_env)
    ...
    describe "without a redirect_uri" do
      it "raises OAuth2::Provider::Rack::InvalidRequest" do
        lambda {
          OAuth2::Provider::Rack::AuthorizationCodeRequest.new(@valid_params.except('redirect_uri'), rack_env)
        }.should raise_error(OAuth2::Provider::Rack::InvalidRequest)
      end
    end
  ...
  context "with realistic data" do
    ...
    describe "Validating requests" do
      action do |env|
        ...
        env['oauth2.authorization_request'] ||= OAuth2::Provider::Rack::AuthorizationCodeRequest.new(request.params, env)
        successful_response
      end
